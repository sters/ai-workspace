# Task: {{DESCRIPTION}}

## Overview

**Task Type**: {{TASK_TYPE}}
**Ticket ID**: {{TICKET_ID}}
**Date**: {{DATE}}

## Workspace Structure

| Path | Description |
|------|-------------|
| `README.md` | Task overview, objectives, requirements, and context. Updated throughout the task. |
| `TODO-{repo}.md` | Checklist of tasks for each repository. Created by workspace-repo-todo-planner agent. |
| `tmp/` | **Temporary directory for agent use.** Agents should use this for intermediate files, test data, scratch work, etc. Contents are gitignored. |
| `reviews/` | Code review reports generated by workspace-repo-review-changes agent. |
| `{org}/{repo}/` | Git worktrees for each repository. Work is done here. |

This workspace is a git repository. Changes to `README.md`, `TODO-*.md`, and `reviews/` are tracked. Use `git log` to view history.

**Gitignored:** `tmp/`, `*.tmp`, `*.log`, repository worktrees (`github.com/`, `gitlab.com/`, `bitbucket.org/`)

## Repositories

<!-- Fill in before running setup-repository.sh -->

## Objective

<!-- Describe what needs to be accomplished -->

## Context

<!-- Background information and why this task is needed -->

## Requirements

<!-- Specific requirements and acceptance criteria -->

## Related Resources

<!-- Links to issues, documentation, etc. -->
